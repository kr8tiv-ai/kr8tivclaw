...
Living backlog of platform-specific problems we want built into the kr8tiv fork/distribution layer.
...
3. **Arena task reliability hardening**
   - Symptom: silent/partial failures when agents are unavailable or responses are delayed.
   - Need: participant preflight, bounded polling with backoff, explicit fail-fast behavior.

## P1

4. **Mission Control task mode bootstrap in generated tenant artifacts**
...
5. **Built-in “ops health snapshot” output**
...
6. **PR cleanup automation for codex branch storms**
